document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupMyTrips() {
    ...
    const notificationReadPill = document.getElementById('notification-read-pill');
    const notificationTitle = document.getElementById('notification-title');
    const notificationSummary = document.getElementById('notification-summary');
    const notificationTime = document.getElementById('notification-time');
    const notificationToggleReadBtn = document.getElementById('notification-toggle-read-btn');
    const notificationItems = Array.from(document.querySelectorAll('.notification-item'));

    const notificationEventKey = `${offerId}:${decision || 'submitted'}:${passengers}`;
    const notificationReadStorageKey = 'nl_rider_notifications_read';
    const notificationTimeStorageKey = 'nl_rider_notifications_time';
    ...
    const readStateMap = readStorageObject(notificationReadStorageKey);
    const timeStateMap = readStorageObject(notificationTimeStorageKey);
    if (!timeStateMap[notificationEventKey]) {
      timeStateMap[notificationEventKey] = new Date().toISOString();
      writeStorageObject(notificationTimeStorageKey, timeStateMap);
    }

    let isRead = Boolean(readStateMap[notificationEventKey]);

    if (notificationTitle) {
      notificationTitle.textContent = 'Join Request Submitted';
    }
    if (notificationSummary) {
      notificationSummary.textContent = 'Your join request was submitted. Full details are available in trip records.';
    }
    if (decision === 'accepted') {
      ...
      if (notificationSummary) {
        notificationSummary.textContent = 'Your join request was accepted. Full details are available in trip records.';
      }
      ...
      if (notificationSummary) {
        notificationSummary.textContent = 'Your join request was rejected. Full details are available in trip records.';
      }
    }

    if (notificationTime) {
      const storedTime = new Date(timeStateMap[notificationEventKey]);
      notificationTime.textContent = Number.isNaN(storedTime.getTime())
        ? 'Just now'
        : storedTime.toLocaleString();
    }

    function applyNotificationReadState() {
      if (notificationReadPill) {
        notificationReadPill.className = isRead ? 'pill confirmed notification-read-pill' : 'pill pending notification-read-pill';
        notificationReadPill.textContent = isRead ? 'Read' : 'Unread';
      }
      if (notificationToggleReadBtn) {
        notificationToggleReadBtn.textContent = isRead ? 'Mark as Unread' : 'Mark as Read';
      }
    }

    const primaryNotification = document.getElementById('join-request-submitted');
    if (primaryNotification) {
      primaryNotification.setAttribute('data-notification-key', notificationEventKey);
    }

    notificationToggleReadBtn?.addEventListener('click', () => {
      isRead = !isRead;
      readStateMap[notificationEventKey] = isRead;
      writeStorageObject(notificationReadStorageKey, readStateMap);
    ...
    notificationItems.forEach((item, index) => {
      const itemKey = item.getAttribute('data-notification-key') || `notification-${index}`;
      const defaultRead = item.getAttribute('data-default-read') === 'true';
      const itemPill = item.querySelector('.notification-read-pill');
      const itemButton = item.querySelector('.notification-toggle-read-btn');

      if (item.id === 'join-request-submitted') {
        return;
      }

      let itemRead = Object.prototype.hasOwnProperty.call(readStateMap, itemKey)
        ? Boolean(readStateMap[itemKey])
        : defaultRead;

      function renderItemRead() {
        if (itemPill) {
          itemPill.className = itemRead ? 'pill confirmed notification-read-pill' : 'pill pending notification-read-pill';
          itemPill.textContent = itemRead ? 'Read' : 'Unread';
        }
        if (itemButton) {
          itemButton.textContent = itemRead ? 'Mark as Unread' : 'Mark as Read';
        }
      }

      itemButton?.addEventListener('click', () => {
        itemRead = !itemRead;
        readStateMap[itemKey] = itemRead;
        writeStorageObject(notificationReadStorageKey, readStateMap);
        renderItemRead();
      });
